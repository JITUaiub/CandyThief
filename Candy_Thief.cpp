...
|             * Game Name: Pixel Art Text Generator, Source: codepen.io          |
...
void gameOver()
{
    ...
    screen_pos_x = glutGet((GLenum)GLUT_WINDOW_X) - 8;
    screen_pos_y = glutGet((GLenum)GLUT_WINDOW_Y) - 31;
    ...
    glRecti(176, 62, 688, 112);

    glColor3ub(0,0,0);
    text = "Exit";
    DrawText(text.data(), text.size(), 400, 81);

    glColor3ub(205, 92, 92);
    glRecti(176, 127, 688, 177);

    glColor3ub(0,0,0);
    text = "Try Again";
    DrawText(text.data(), text.size(), 380, 145);

    glFlush();
}

void game_quit_keyboard1(unsigned char key, int x, int y)
{
    switch(key)
    {
        case 'Q':
...
///Game Over Motion
void windowposition2(int xp, int yp)
{
...
void mouse2(int button, int state, int x, int y)
{
    ...
    switch(button)
    {
        case GLUT_LEFT_BUTTON:
            if(state==GLUT_DOWN)
            {
                if(mouseX >=176 && mouseX <= 688)
                    if(mouseY >= 428 && mouseY <= 476)
                    {
...
void gameOverDisplay(void)
{
    ...
    glutDestroyWindow(display_count);
    ...
    glutKeyboardFunc(game_quit_keyboard1);
    glutMouseFunc(mouse2);
    glutMotionFunc(windowposition2);
    glutMainLoop();
}
...
static void Play()
{
	...
	ShowCursor(true);
	GetCursorPos(&mpos);
	mpos.x -= screen_pos_x; mpos.y -=screen_pos_y;
    background();
    if (lifeGone <= 10)
        {
            glPointSize(2);
    ...
    switch(button)
    {
        case GLUT_LEFT_BUTTON:
            if(state==GLUT_DOWN)
            {
                if(s == 1)
                {
                    if(mouseX >=256 && mouseX <= 768) ///Play
                            if(mouseY >= 288 && mouseY <= 350)
                            {
                                s = 4;
                                glutDisplayFunc(nnew);
                            }
                    else if(mouseX >=256 && mouseX <= 768) /// help
                            if(mouseY >= 358 && mouseY <= 408)
                            {
                                s = 3;
                            }
                    else if(mouseX >=256 && mouseX <= 768) /// credits
                            if(mouseY >= 423 && mouseY <= 473)
                            {
                                s = 2;
                            }
                    else if(mouseX >=256 && mouseX <= 768) ///Exit
                            if(mouseY >= 492 && mouseY <= 538)
                            {
                                s = 0;
                            }
                ...
                if(s == 2 || s == 3)
                {
                    if(mouseX >=250 && mouseX <= 550) ///Back
                            if(mouseY >= 505 && mouseY <= 560)
                            {
                                s = 1;
                            }
                }
...
{
    /// ------------------------------ Main Function -----------------------------
    PlaySound(TEXT("menuMusic.wav"), NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
    ifstream in;
